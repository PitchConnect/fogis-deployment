...
class MultiPlatformTester:
        ...
        try:
            ...
            if manifest.get("mediaType") == "application/vnd.docker.distribution.manifest.list.v2+json":
                ...
                for platform in self.platforms:
                    ...
                    platform_found = any(
                        m.get("platform", {}).get("os") == os_name and
    ...
    def check_image_platforms_alternative(self, image: str) -> Dict[str, bool]:
        ...
        for platform in self.platforms:
            ...
            exit_code, stdout, stderr = self.run_command([
                "docker", "pull", "--platform", platform, f"{self.registry}/{image}:latest"
            ], timeout=120)
    ...
    def test_current_platform_functionality(self, image: str) -> bool:
        ...
        exit_code, stdout, stderr = self.run_command([
            ...
            f"{self.registry}/{image}:latest", "test"
        ], timeout=30)
    ...
    def generate_platform_report(self) -> str:
        ...
        for service in self.services:
            ...
            report.append(f"| {service} | {amd64_support} | {arm64_support} | {functional} |")
        
        report.append("")
        
        # Summary
        total_services = len(self.services)
        amd64_count = sum(1 for s in self.services if self.results.get(s, {}).get("linux/amd64", False))
        arm64_count = sum(1 for s in self.services if self.results.get(s, {}).get("linux/arm64", False))
        functional_count = sum(1 for s in self.services if self.results.get(s, {}).get("functional", False))
        ...
        report.append(f"- **Functional Tests**: {functional_count}/{total_services} services")
    ...
    def run_all_tests(self) -> bool:
        ...
        for service in self.services:
            ...
            service_passed = (
                platform_support.get("linux/amd64", False) and
                platform_support.get("linux/arm64", False) and
                functional
            )
