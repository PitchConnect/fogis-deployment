        ...
        try:
            ...
            if login_response.status_code == 200:
                ...
                if any(indicator in response_text for indicator in [
                    'invalid', 'incorrect', 'failed', 'error', 'fel'
                ]):
                ...
                if matches_response.status_code == 200:
                    return True, "FOGIS authentication successful", {
                        'status_code': matches_response.status_code,
                        'authenticated': True,
                        'response_length': len(matches_response.text)
                    }
                else:
                    return False, f"Authentication failed: Unable to access protected resource (HTTP {matches_response.status_code})", {
                        'status_code': matches_response.status_code,
                        'authenticated': False
                    }
            else:
            ...
            return False, "Authentication failed: Connection error", {'error': 'connection'}
        except requests.exceptions.RequestException as e:
            return False, f"Authentication failed: {str(e)}", {'error': str(e)}
        except Exception as e:
            return False, f"Unexpected error during authentication: {str(e)}", {'error': str(e)}
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ['-h', '--help']:
            ...
            print("Environment variables FOGIS_USERNAME and FOGIS_PASSWORD are also supported")
            sys.exit(0)
        elif len(sys.argv) >= 3:
            username = sys.argv[1]
            password = sys.argv[2]
