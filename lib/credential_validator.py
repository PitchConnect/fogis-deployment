...
class CredentialValidator:
    ...
    def validate_google_credentials(
        self, credentials_file: str = "credentials.json"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if credentials:
                ...
                all_apis_working = all(
                    api_results.get(api, False) for api in required_apis
                )
                ...
                if all_apis_working:
                    ...
                    failed_apis = [
                        api for api in required_apis if not api_results.get(api, False)
                    ]
    ...
    def validate_fogis_credentials(
        ...
        try:
            ...
            if auth_result["success"]:
                ...
                self.logger.info(
                    f"FOGIS credentials validation successful for user: {username}"
                )
            else:
    ...
    def validate_calendar_setup(
        self, calendar_id: str, credentials_file: str = "credentials.json"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if validation_result["valid"]:
                ...
                if test_event_id:
                    ...
                    self.logger.info(
                        f"Calendar validation successful for: {calendar_id}"
                    )
                else:
        ...
        try:
                ...
                if component in self.validation_results:
                    ...
                    if component_result.get("valid", False):
                        ...
                        result["issues"].append(
                            f"{component}: {component_result.get('error', 'Unknown error')}"
                        )
                else:
                    result["components"][component] = {
                        "status": "not_tested",
            ...
            if result["overall_score"] < 100:
                result["recommendations"].append(
                    "Run credential setup wizard to fix issues"
                )
                ...
                if "fogis_auth" in result["issues"]:
                    result["recommendations"].append(
                        "Verify FOGIS username and password"
                    )
            ...
            self.logger.info(
                f"Integration health check completed - Score: {result['overall_score']}%"
            )
    ...
    def run_comprehensive_validation(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "google_credentials" in config:
                google_result = self.validate_google_credentials(
            ...
            if "fogis_username" in config and "fogis_password" in config:
                fogis_result = self.validate_fogis_credentials(
                    config["fogis_username"], config["fogis_password"]
                )
            ...
            self.logger.info(
                f"Comprehensive validation completed - {results['summary']['passed_tests']}/{results['summary']['total_tests']} tests passed"
            )
