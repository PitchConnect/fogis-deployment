...
class FogisAuthManager:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"FOGIS connection failed with status: {response.status_code}"
                )
                return False
    ...
    def authenticate(self, username: str, password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if login_page.status_code != 200:
                result[
                    "error"
                ] = f"Could not access login page (status: {login_page.status_code})"
                return result
            ...
            login_response = self.session.post(
                self.login_url, data=login_data, timeout=10, allow_redirects=True
            )
    ...
    def _is_login_successful(self, response: requests.Response) -> bool:
        ...
        if response.url and (
            "dashboard" in response.url.lower() or "profile" in response.url.lower()
        ):
            return True
        ...
        if (
            ...
            and "password" not in response.text.lower()
        ):
            return True
