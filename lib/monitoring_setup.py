...
class MonitoringSetup:
    ...
    def run_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Health check completed. Status: {health_report['overall_status']}"
            )
    ...
    def _check_docker_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if docker_info.returncode != 0:
                return {"status": "error", "error": "Docker daemon not accessible"}
    ...
    def _analyze_performance_trends(
        ...
        trends = {
            "cpu_trend": self._calculate_trend(
                [d["system"]["cpu_percent"] for d in historical_data]
            ),
            "memory_trend": self._calculate_trend(
                [d["system"]["memory_percent"] for d in historical_data]
            ),
            "disk_trend": self._calculate_trend(
                [d["system"]["disk_percent"] for d in historical_data]
            ),
    ...
    def _generate_performance_recommendations(
        ...
        if cpu_percent > 80:
            recommendations.append(
                "High CPU usage detected. Consider scaling up or optimizing services."
            )
        elif cpu_trend.get("direction") == "increasing":
            recommendations.append(
                "CPU usage is trending upward. Monitor for potential scaling needs."
            )
        ...
        if memory_percent > 85:
            recommendations.append(
                "High memory usage detected. Consider increasing memory or optimizing services."
            )
        ...
        if disk_percent > 90:
            recommendations.append(
                "Critical disk usage. Clean up logs and temporary files immediately."
            )
        elif disk_percent > 80:
            recommendations.append(
                "High disk usage. Consider log rotation and cleanup procedures."
            )
        ...
        for service, availability in service_availability.items():
            if availability < 95:
                recommendations.append(
                    f"Service {service} has low availability ({availability}%). Investigate issues."
                )
...
def main():
    ...
    try:
        ...
        if command == "health-check":
            ...
            print(
                f"Services: {len([s for s in report['services'].values() if s.get('status') == 'healthy'])} healthy"
            )
            if report["issues"]:
                print("Issues:")
