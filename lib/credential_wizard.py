...
class CredentialWizard:
    ...
    def _create_new_calendar(self) -> bool:
        ...
        try:
            ...
            if not validation["valid"]:
                self.print_error(
                    f"Calendar validation failed: {validation.get('error', 'Unknown error')}"
                )
                return False
            ...
            if share in ["y", "yes"]:
                ...
                if email and "@" in email:
                    if self.calendar_manager.share_calendar(
                        calendar_id, email, "writer"
                    ):
    ...
    def _use_existing_calendar(self) -> bool:
        ...
        try:
            ...
            while True:
                choice = input(
                    f"\nSelect calendar (1-{len(calendars[:10]) + 1}): "
                ).strip()
                ...
                try:
                    ...
                    if 1 <= choice_num <= len(calendars[:10]):
                        ...
                        print(
                            f"Please enter a number between 1 and {len(calendars[:10]) + 1}"
                        )
                except ValueError:
            ...
            if validation["valid"]:
                self.config["calendar_id"] = calendar_id
                self.config["calendar_name"] = validation["calendar_info"].get(
    ...
    def setup_fogis_auth(self) -> bool:
        ...
        try:
            ...
            if not self.fogis_manager.test_connection():
                self.print_error(
                    "Cannot connect to FOGIS website. Please check your internet connection."
                )
                return False
            ...
            if auth_result["success"]:
                ...
                if detected_referee_number:
                    self.print_info(
                        f"Auto-detected referee ID: {detected_referee_number}"
                    )
                    if detected_referee_number != referee_number:
                        self.print_warning(
                            f"Entered referee number ({referee_number}) differs from detected ({detected_referee_number})"
                        )
                        use_detected = (
                            input("Use auto-detected referee number? (Y/n): ")
                            .strip()
                            .lower()
                        )
                ...
                # Offer retry
                retry = (
                    input("Retry with different credentials? (y/N): ").strip().lower()
                )
                if retry in ["y", "yes"]:
    ...
    def _test_calendar_access(self) -> bool:
        ...
        try:
            ...
            # Validate calendar access
            validation = self.calendar_manager.validate_calendar_access(
    ...
    def completion_summary(self) -> None:
        ...
        if "calendar_id" in self.config:
            print(
                f"   📅 Google Calendar: {self.config.get('calendar_name', 'Configured')}"
            )
        ...
        try:
            ...
            if "fogis_username" in self.config:
                self._update_env_var(
                    env_content, "FOGIS_USERNAME", self.config["fogis_username"]
                )
                self._update_env_var(
                    env_content, "FOGIS_PASSWORD", self.config["fogis_password"]
                )
                self._update_env_var(
                    env_content, "USER_REFEREE_NUMBER", self.config["referee_number"]
                )

            # Update calendar configuration
            if "calendar_id" in self.config:
                self._update_env_var(
                    env_content, "GOOGLE_CALENDAR_ID", self.config["calendar_id"]
                )
