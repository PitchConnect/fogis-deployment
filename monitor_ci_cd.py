...
class CICDMonitor:
    ...
    def analyze_build_performance(self, runs: List[Dict]) -> Dict:
        ...
        successful_runs = len(
            [r for r in completed_runs if r["conclusion"] == "success"]
        )
    ...
    def generate_health_report(self, metrics: Dict, failures: List[Dict]) -> str:
        ...
        report.append(
            f"- **Average Duration**: {metrics.get('average_duration_minutes', 0):.1f} minutes"
        )
        report.append("")
    ...
    def run_monitoring(self, days: int = 7) -> bool:
        ...
        # Save report
        report_file = (
            f"ci_cd_health_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
...
def main():
    ...
    parser.add_argument(
        "--repo", default="PitchConnect/fogis-deployment", help="Repository to monitor"
    )
