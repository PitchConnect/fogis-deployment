...
class SetupWizard:
    ...
    def run_setup(self, interactive: bool = True) -> bool:
        ...
        if interactive:
            ...
            print(
                "Each step will be validated and fixed automatically when possible.\n"
            )
    ...
    def _run_step(self, step: SetupStep, interactive: bool) -> bool:
        ...
        try:
            ...
            if step.required:
                print(
                    f"   This step is required. Please fix manually and re-run setup."
                )
                return False
            else:
    ...
    def _check_docker_compose(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                result.returncode == 0,
                "" if result.returncode == 0 else "Docker Compose not found",
            )
        except FileNotFoundError:
    ...
    def _check_git(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                result.returncode == 0,
                "" if result.returncode == 0 else "Git command failed",
            )
        except FileNotFoundError:
    ...
    def _check_fogis_credentials(self) -> Tuple[bool, str]:
        ...
        lines = content.split("\n")
        username_set = any(
            line.startswith("FOGIS_USERNAME=")
            and "=" in line
            and line.split("=", 1)[1].strip()
            for line in lines
        )
        password_set = any(
            line.startswith("FOGIS_PASSWORD=")
            and "=" in line
            and line.split("=", 1)[1].strip()
            for line in lines
        )
