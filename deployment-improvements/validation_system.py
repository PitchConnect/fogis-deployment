...
class FOGISValidator:
    ...
    def validate_deployment(self, comprehensive: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            results["services"] = {name: asdict(health) for name, health in service_health.items()}
            ...
            results["tests"].append(ValidationResult(
                "validation_system", False, f"System error: {e}", 0
            ))
        ...
        try:
            ...
            if result.returncode != 0:
                return ValidationResult(
                    "docker_environment", False, "Docker is not running", 
            ...
            expected_services = len([s for s in self.services_config.values() if s.get("required", True)])
            ...
            return ValidationResult(
                "docker_environment", True, "Docker environment is healthy",
                (time.time() - start_time) * 1000,
                {"running_services": running_services}
            )
            ...
            return ValidationResult(
                "docker_environment", False, f"Docker check failed: {e}",
                (time.time() - start_time) * 1000
            )
    ...
    def _test_detector_processor_interaction(self) -> ValidationResult:
        ...
        try:
            ...
            if not trigger_url:
                return ValidationResult(
                    "detector_processor_interaction", False,
                    "Trigger URL not configured", (time.time() - start_time) * 1000
                )
            ...
            if response.status_code == 200:
                return ValidationResult(
                    "detector_processor_interaction", True,
                    "Detector trigger successful", (time.time() - start_time) * 1000,
            ...
            return ValidationResult(
                "detector_processor_interaction", False,
                f"Interaction test failed: {e}", (time.time() - start_time) * 1000
            )
    ...
    def _test_processor_calendar_interaction(self) -> ValidationResult:
        ...
        try:
            calendar_config = self.services_config.get("fogis-calendar-phonebook-sync", {})
            health_url = calendar_config.get("health_url")
            
            if health_url:
                response = requests.get(health_url, timeout=10)
                success = response.status_code == 200
                message = "Calendar service reachable" if success else f"HTTP {response.status_code}"
            else:
                success = False
                message = "Calendar service not configured"
            
            return ValidationResult(
                "processor_calendar_interaction", success, message,
                (time.time() - start_time) * 1000
            )
            
        except Exception as e:
            return ValidationResult(
                "processor_calendar_interaction", False,
                f"Calendar interaction test failed: {e}", (time.time() - start_time) * 1000
            )
    ...
    def _test_processor_drive_interaction(self) -> ValidationResult:
        ...
        try:
            ...
            if health_url:
                response = requests.get(health_url, timeout=10)
                success = response.status_code == 200
                message = "Drive service reachable" if success else f"HTTP {response.status_code}"
            else:
                success = False
                message = "Drive service not configured"
            
            return ValidationResult(
                "processor_drive_interaction", success, message,
                (time.time() - start_time) * 1000
            )
            
        except Exception as e:
            return ValidationResult(
                "processor_drive_interaction", False,
                f"Drive interaction test failed: {e}", (time.time() - start_time) * 1000
            )
    ...
    def _validate_end_to_end_pipeline(self) -> ValidationResult:
        ...
        try:
            ...
            required_services = [
                name for name, config in self.services_config.items() 
                if config.get("required", True)
            ]
            
            healthy_services = 0
            for service_name in required_services:
                health = self._check_service_health(service_name, self.services_config[service_name])
                if health.status == HealthStatus.HEALTHY:
                    healthy_services += 1
            
            success = healthy_services == len(required_services)
            message = f"{healthy_services}/{len(required_services)} required services healthy"
            
            return ValidationResult(
                "end_to_end_pipeline", success, message,
                (time.time() - start_time) * 1000,
                {"healthy_services": healthy_services, "total_required": len(required_services)}
            )
            
        except Exception as e:
            return ValidationResult(
                "end_to_end_pipeline", False,
                f"Pipeline validation failed: {e}", (time.time() - start_time) * 1000
            )
    ...
    def _validate_resource_usage(self) -> ValidationResult:
        ...
        try:
            ...
            if result.returncode != 0:
                return ValidationResult(
                    "resource_usage", False, "Failed to get Docker stats",
                    (time.time() - start_time) * 1000
                )
            ...
            return ValidationResult(
                "resource_usage", success, message,
                (time.time() - start_time) * 1000,
                {"high_cpu": high_cpu_containers, "high_memory": high_memory_containers}
            )
            
        except Exception as e:
            return ValidationResult(
                "resource_usage", False, f"Resource check failed: {e}",
                (time.time() - start_time) * 1000
            )
    ...
    def continuous_monitoring(self, interval_seconds: int = 60, duration_minutes: int = 60) -> None:
...
def main():
    ...
    if args.monitor:
        ...
        sys.exit(0 if results["overall_status"] in [HealthStatus.HEALTHY, HealthStatus.DEGRADED] else 1)
