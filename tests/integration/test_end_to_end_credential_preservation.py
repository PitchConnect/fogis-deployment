...
"""
            ...
            script_source = Path(__file__).parent.parent.parent / "restore_fogis_credentials.sh"
            if script_source.exists():
                shutil.copy(script_source, deployment_dir)
            
            # Copy validation script
            validation_source = Path(__file__).parent.parent.parent / "scripts" / "validate_fogis_credentials.py"
    ...
    def test_backup_creation_and_restoration_workflow(self, test_environment):
        ...
        backup_script = Path(__file__).parent.parent.parent / "create_credential_backup.sh"
        if backup_script.exists():
            ...
            if restoration_script.exists():
                ...
                assert result.returncode == 0, f"Restoration should succeed: {result.stderr}"
...
"""
        ...
        if restoration_script.exists():
            ...
            assert "Successfully updated .env" in result.stdout or "Successfully updated .env" in result.stderr
...
"""
    ...
    def test_backup_detection_and_auto_restoration(self, test_environment):
        ...
        (backup1 / "FOGIS_CREDENTIALS.txt").write_text("FOGIS_USERNAME=Old User\nFOGIS_PASSWORD=old_pass")
        (backup2 / "FOGIS_CREDENTIALS.txt").write_text("FOGIS_USERNAME=New User\nFOGIS_PASSWORD=new_pass")
    ...
    def test_error_handling_missing_backup(self, test_environment):
        ...
        if restoration_script.exists():
            ...
            assert "No backup directory found" in result.stdout or "No backup directory found" in result.stderr
    ...
    def test_env_file_backup_and_restoration(self, test_environment):
        ...
        credentials_file = backup_dir / "FOGIS_CREDENTIALS.txt"
        credentials_file.write_text("FOGIS_USERNAME=Backup User\nFOGIS_PASSWORD=backup_pass")
        ...
        if restoration_script.exists():
            ...
            if result.returncode == 0:
                ...
                assert len(backup_files) > 0, "Backup of original .env should be created"
                
                # Verify backup contains original content
                backup_content = backup_files[0].read_text()
                assert "Original User" in backup_content
    
    @patch.dict(os.environ, {'FOGIS_USERNAME': 'env_user', 'FOGIS_PASSWORD': 'env_pass'})
    def test_environment_variable_precedence(self, test_environment):
        ...
        if validation_script.exists():
            result = subprocess.run(
                ...
                text=True,
                timeout=30,
                env={**os.environ, 'FOGIS_USERNAME': 'env_user', 'FOGIS_PASSWORD': 'env_pass'}
            )
