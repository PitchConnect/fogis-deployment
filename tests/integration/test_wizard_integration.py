...
class TestCredentialWizardIntegration:
    ...
    def test_complete_wizard_flow_success(self, mock_exists, mock_storage_class, 
                                        mock_fogis_class, mock_calendar_class, 
                                        mock_oauth_class, mock_input):
        ...
        mock_oauth.test_credentials.return_value = {
            'calendar': True, 'drive': True, 'contacts': True
        }
        ...
        mock_calendar.validate_calendar_access.return_value = {
            'valid': True, 'calendar_info': {'summary': 'Test Calendar'}
        }
        ...
        with patch('getpass.getpass', return_value='testpass'), \
             patch.object(self.wizard, '_update_env_file'), \
    ...
    def test_fogis_manager_integration(self, mock_fogis_class):
        ...
        # Test FOGIS setup
        with patch('builtins.input', side_effect=['testuser', '12345']), \
    ...
    def test_env_file_operations(self):
        ...
        with patch('os.path.exists', return_value=True), \
             patch('builtins.open', mock_open(read_data='EXISTING_VAR=existing_value\nFOGIS_USERNAME=old_user\n')):
    ...
    @patch('lib.credential_wizard.GoogleOAuthManager')
    @patch('lib.credential_wizard.CalendarManager')
    def test_calendar_creation_flow(self, mock_calendar_class, mock_oauth_class, mock_input):
        ...
        mock_calendar.validate_calendar_access.return_value = {
            'valid': True, 'calendar_info': {'summary': 'FOGIS Matches - 2024'}
        }
        mock_calendar_class.return_value = mock_calendar
    ...
    def test_service_credentials_setup(self, mock_exists, mock_makedirs, mock_copy):
        ...
        mock_copy.assert_any_call('token.json', 'data/fogis-calendar-phonebook-sync/token.json')
        mock_copy.assert_any_call('token.json', 'data/google-drive-service/google-drive-token.json')
