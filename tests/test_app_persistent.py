...
class TestPersistentMatchListProcessorApp:
    ...
    def app_instance(self, mock_services):
        ...
        with patch('src.app_persistent.DockerNetworkApiClient', return_value=mock_services['api_client']), \
             patch('src.app_persistent.FogisPhonebookSyncService', return_value=mock_services['phonebook_service']), \
             patch('src.app_persistent.MatchDataManager', return_value=mock_services['data_manager']), \
             patch('src.app_persistent.MatchComparator', return_value=mock_services['match_comparator']), \
             patch('src.app_persistent.MatchProcessor', return_value=mock_services['match_processor']), \
             patch('src.app_persistent.WhatsAppAvatarService', return_value=mock_services['avatar_service']), \
             patch('src.app_persistent.GoogleDriveStorageService', return_value=mock_services['storage_service']):
    ...
    def test_init(self):
        ...
        with patch('src.app_persistent.DockerNetworkApiClient'), \
