...
class TestMinimalCredentialWizard:
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("builtins.input")
    def test_setup_google_oauth_invalid_format(
        self, mock_input, mock_file, mock_exists
    ):
    ...
    def test_run_calendar_failure(self):
        ...
        with patch.object(
            ...
            self.wizard, "setup_calendar_integration", return_value=False
        ):
            result = self.wizard.run()
            assert result == 1
    ...
    def test_run_fogis_failure(self):
        ...
        with patch.object(
            ...
            self.wizard, "setup_fogis_auth", return_value=False
        ):
            result = self.wizard.run()
            assert result == 1
    ...
    def test_run_save_failure(self):
        ...
        with patch.object(
            ...
            self.wizard, "save_configuration", return_value=False
        ):
            result = self.wizard.run()
            assert result == 1
    ...
    def test_run_unexpected_exception(self):
        ...
        with patch.object(
            self.wizard, "welcome_screen", side_effect=Exception("Unexpected error")
        ):
            result = self.wizard.run()
            assert result == 1
