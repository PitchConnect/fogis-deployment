...
class TestInteractiveSetup(unittest.TestCase):
    ...
    def test_initialize_config_structure(self):
        ...
        self.assertIn(
            "Interactive Setup", self.setup_wizard.config["metadata"]["description"]
        )
    ...
    def test_setup_google_oauth(self, mock_input, mock_oauth_wizard_class):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.setup_wizard.config["google"]["oauth"]["client_type"],
            "web_application",
        )
        self.assertEqual(
            self.setup_wizard.config["google"]["calendar"]["calendar_id"], "primary"
        )
        self.assertEqual(
            self.setup_wizard.config["google"]["drive"]["folder_base"],
            "WhatsApp_Assets",
        )
    ...
    def test_setup_service_configuration(self, mock_input, mock_ask):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.setup_wizard.config["services"]["processing"][
                "min_referees_for_whatsapp"
            ],
            2,
        )
        self.assertEqual(
            self.setup_wizard.config["services"]["logging"]["level"], "INFO"
        )
    ...
    def test_setup_system_settings(self, mock_input, mock_ask):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.setup_wizard.config["system"]["docker"]["restart_policy"],
            "unless-stopped",
        )
    ...
    def test_validate_cron_schedule(self):
        ...
        self.assertFalse(
            self.setup_wizard.validate_cron_schedule("0 * * *")
        )  # Too few fields
        self.assertFalse(
            self.setup_wizard.validate_cron_schedule("0 * * * * *")
        )  # Too many fields
    ...
    def test_setup_custom_ports(self, mock_input):
        ...
        self.assertEqual(
            self.setup_wizard.config["services"]["ports"]["logo_combiner"], 9999
        )
        # Check that defaults were kept for others
        self.assertEqual(
            self.setup_wizard.config["services"]["ports"]["api_client"], 9086
        )
    ...
    def test_setup_email_notifications(self, mock_input, mock_ask):
        ...
        self.assertEqual(
            self.setup_wizard.config["notifications"]["email"]["sender"],
            "sender@test.com",
        )
        self.assertEqual(
            self.setup_wizard.config["notifications"]["email"]["receiver"],
            "receiver@test.com",
        )
        self.assertEqual(
            self.setup_wizard.config["notifications"]["email"]["smtp_server"],
            "smtp.test.com",
        )
        self.assertEqual(
            self.setup_wizard.config["notifications"]["email"]["smtp_port"], 587
        )
