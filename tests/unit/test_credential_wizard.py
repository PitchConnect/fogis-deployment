...
class TestCredentialWizard:
    ...
    def test_setup_google_oauth_no_credentials_file(
        self, mock_oauth_manager, mock_exists
    ):
    ...
    @patch("lib.credential_wizard.GoogleOAuthManager")
    @patch("builtins.input")
    def test_setup_google_oauth_success(
        self, mock_input, mock_oauth_manager, mock_exists
    ):
    ...
    @patch("os.path.exists")
    @patch("lib.credential_wizard.GoogleOAuthManager")
    def test_setup_google_oauth_invalid_credentials(
        self, mock_oauth_manager, mock_exists
    ):
    ...
    def test_setup_fogis_auth_success(
        self, mock_fogis_manager, mock_getpass, mock_input
    ):
    ...
    def test_validate_and_test_some_fail(self):
        ...
        with patch.object(
            ...
            self.wizard, "_test_calendar_access", return_value=False
        ), patch.object(
            self.wizard, "_test_fogis_auth", return_value=True
        ), patch.object(
            self.wizard, "_test_credential_storage", return_value=True
    ...
    def test_test_credential_storage_success(self, mock_storage_class):
        ...
        mock_storage.retrieve_credentials.return_value = {
            "test": "data",
            "timestamp": 123456,
        }
    ...
    def test_save_configuration_success(self, mock_storage_class):
        ...
        with patch.object(self.wizard, "_update_env_file"), patch.object(
    ...
    def test_run_complete_success(self, mock_input):
        ...
        with patch.object(
            self.wizard, "welcome_screen", return_value=True
        ), patch.object(
            self.wizard, "setup_google_oauth", return_value=True
        ), patch.object(
            self.wizard, "setup_calendar_integration", return_value=True
        ), patch.object(
            self.wizard, "setup_fogis_auth", return_value=True
        ), patch.object(
            self.wizard, "validate_and_test", return_value=True
        ), patch.object(
            self.wizard, "save_configuration", return_value=True
        ), patch.object(
