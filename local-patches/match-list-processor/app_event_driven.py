...
class EventDrivenMatchListProcessor:
    ...
    def _create_app(self) -> FastAPI:
        ...
        async def root():
            ...
            return {
                ...
                "mode": "event-driven",
                "version": "1.0.0",
                "endpoints": {"health": "/health", "process": "/process (POST)"},
    ...
    def _process_matches_sync(self) -> None:
        ...
        try:
            ...
            if not sync_result:
                logger.warning(
                    "Contact sync failed, but continuing with match processing."
                )

            # Load and parse previous matches
            previous_matches_dict = self._load_previous_matches()
            logger.info(
                f"Loaded previous matches data: {len(previous_matches_dict)} matches."
            )
            ...
            logger.info(
                f"Fetched current matches data: {len(current_matches_dict)} matches."
            )
            ...
            if changes["has_changes"]:
                logger.info(
                    f"Processing {len(changes['new_matches'])} new matches and {len(changes['updated_matches'])} updated matches"
                )

                # Process new and updated matches
                all_matches_to_process = list(changes["new_matches"].values()) + list(
                    changes["updated_matches"].values()
                )
                ...
                for match in all_matches_to_process:
                    try:
                        ...
                        logger.info(
                            f"Successfully processed match {match.get('match_id', 'unknown')}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to process match {match.get('match_id', 'unknown')}: {e}"
                        )
            else:
            ...
            self.data_manager.save_previous_matches_raw_json(
                self.data_manager.convert_list_to_raw_json(
                    list(current_matches_dict.values())
                )
            )
